...
<head>
    ...
    <title>MindBalancer Dashboard</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .server-info h4 { font-size: 14px; font-weight: 600; margin-bottom: 2px; }
        .server-info span { font-size: 12px; color: var(--text-muted); }

        .server-stat { text-align: center; padding: 0 12px; }
        .server-stat-value { font-size: 16px; font-weight: 600; font-family: 'JetBrains Mono', monospace; }
        ...
        .badge-openai { background: var(--accent-green-dim); color: var(--accent-green); }
        .badge-anthropic { background: var(--accent-purple-dim); color: var(--accent-purple); }
        .badge-ollama { background: var(--accent-blue-dim); color: var(--accent-blue); }

        .health-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; }

        .health-item { background: var(--bg-secondary); border-radius: 10px; padding: 16px; }
        .health-item-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
        .health-item-name { font-size: 13px; font-weight: 500; }
        .health-item-status { font-size: 11px; font-weight: 600; padding: 2px 8px; border-radius: 4px; }
        .health-item-status.healthy { background: var(--accent-green-dim); color: var(--accent-green); }
        .health-item-status.unhealthy { background: var(--accent-red-dim); color: var(--accent-red); }
        .health-item-latency { font-size: 20px; font-weight: 600; font-family: 'JetBrains Mono', monospace; }
        ...
        @media (max-width: 768px) {
            ...
            .server-item { grid-template-columns: auto 1fr; gap: 12px; }
    ...
    <div class="container">
        <header>
            <div class="logo">
                ...
                <div>
                    <h1>MindBalancer</h1>
                    <span>AI Load Balancer Dashboard</span>
                </div>
            </div>
        ...
        <div class="main-grid">
            <div class="card">
                <div class="card-header">
                    ...
                    <span id="algorithmBadge" class="badge badge-openai">weighted_round_robin</span>
                </div>
                <div class="card-body">
    ...
    <script>
        async function fetchStats() {
            try {
                ...
                if (latencyCount > 0) {
                    const avgMs = (totalLatency / latencyCount / 1000000).toFixed(0);
        ...
        function renderServers(servers) {
            ...
            container.innerHTML = servers.map(server => {
                const isHealthy = server.Errors === 0 || server.TotalReqs === 0;
                const latencyMs = server.AvgLatency > 0 ? (server.AvgLatency / 1000000).toFixed(0) : '-';
                const providerClass = getProviderClass(server.Name);
                
                return '<div class="server-item">' +
                    '<div class="server-status ' + (isHealthy ? 'online' : 'offline') + '"></div>' +
                    '<div class="server-info">' +
                    '<h4>' + server.Name + '</h4>' +
                    '<span>Hostgroup ' + server.Hostgroup + ' • Weight ' + server.Weight + '</span>' +
                    '</div>' +
                    '<div class="server-stat">' +
                    '<div class="server-stat-value">' + server.TotalReqs + '</div>' +
                    '<div class="server-stat-label">Requests</div>' +
                    '</div>' +
                    '<div class="server-stat">' +
                    '<div class="server-stat-value">' + latencyMs + 'ms</div>' +
                    '<div class="server-stat-label">Latency</div>' +
                    '</div>' +
                    '<span class="badge ' + providerClass + '">' + getProviderName(server.Name) + '</span>' +
                    '</div>';
            }).join('');
        }
        ...
        function renderHealth(healthData) {
            ...
            container.innerHTML = entries.map(function(entry) {
                ...
                const latencyMs = status.Latency > 0 ? (status.Latency / 1000000).toFixed(0) : '-';
                return '<div class="health-item">' +
                    '<div class="health-item-header">' +
                    '<span class="health-item-name">' + name + '</span>' +
                    '<span class="health-item-status ' + (status.Healthy ? 'healthy' : 'unhealthy') + '">' +
                    (status.Healthy ? 'Healthy' : 'Unhealthy') +
                    '</span>' +
                    '</div>' +
                    '<div class="health-item-latency">' + latencyMs + 'ms</div>' +
                    '</div>';
            }).join('');
        }
